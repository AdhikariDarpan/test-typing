...
function getRandomUniqueParagraph(paragraphList, lang, level) {
  const key = `${lang}-${level}`;
  if (!usedParagraphs[key]) {
    usedParagraphs[key] = new Set();
  }
  if (usedParagraphs[key].size === paragraphList.length) {
    usedParagraphs[key].clear();
  }
  let randomIndex;
  do {
    randomIndex = Math.floor(Math.random() * paragraphList.length);
  } while (usedParagraphs[key].has(randomIndex));
  usedParagraphs[key].add(randomIndex);
...
function fetchParagraph(language, level) {
  try {
    ...
    if (level == "extreme") {
      ...
      originalText = randomizeWords(
        getRandomUniqueParagraph(paragraphList, language,level)
      );
    } else {
      ...
      originalText = getRandomUniqueParagraph(paragraphList, language,level);
    }
    ...
    paragraphEl.textContent = `Error fetching paragraph. Please select another language.`;
    resetInputBox(true);
  }
}
...
function displayResults() {
  ...
  const wpm = Math.round((correctWords / totalTime));  
  const ccpm = Math.round(correctChars / totalTime / 60); 
  let wrongWords = wordCount - correctWords;
  let accuracy = ((correctChars / (correctChars + incorrectCharacters)) * 100).toFixed(2);
  if (inputBox.value.trim() === '') {
    wrongWords = paragraphEl.innerText.trim().split(/\s+/).length;
    accuracy = 0;
  }
  resultsEl.innerHTML = `
